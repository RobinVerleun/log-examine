...
class Validator:
	...
	def validate_timestamp(self, time):
		...
		try:
			num_time = int(time)
			timestamp = datetime.fromtimestamp(num_time)
		except:
			self.valid = False
	...
	def createRegexPattern(self, version):
	...
	def __call__(self, file, linenum):
		# Set the valid flag to true at the beginning of the validation pass
		# Clear the errors buffer
		self.reset_state()

		self.validate_timestamp(file['ts'])
		self.validate_time(file['pt'])
		self.validate_sha(file['sha'])
		self.validate_path(file['ph'], file['nm'])
		self.validate_disposition(file['dp'])

		all_pattern = self.createRegexPattern('[0-5]')
		self.validate_UUID(file['si'], all_pattern)
		self.validate_UUID(file['uu'], all_pattern)
		self.validate_UUID(file['bg'], all_pattern)

		filename = self.validate_file_name(file['nm'])
